...
def extract_data_from_html(content):
    ...
    # "観光・PR"がテキストに含まれていないことを確認する条件に変更
    if soup.find(class_="sorting-list__item", string=lambda text: text and "観光・PR" in text):
        return None
    ...
    if (quantity):
        amount = quantity.text.strip()
        # カンマを取り除く
        cleaned_amount = amount.replace(',', '')
        data['寄付金額'] = cleaned_amount
    else:
        data['寄付金額'] = ''
    ...
    if target_amount:
        target_amount_text = target_amount.text.strip().replace('円', '').replace(',', '').strip().replace('目標金額：', '').replace(',', '')
        data['目標金額'] = target_amount_text
    else:
        data['目標金額'] = ''
    ...
    if period:
        ...
        full_period = ''.join(filter(str.isdigit, days))
        # 下二桁を取得して半角に変換
        last_two_digits = full_period[-2:]
        data['期間'] = last_two_digits
    else:
        data['期間'] = ''
    ...
    data['進捗情報'] = progress.text.strip() if progress else '0'
...
if __name__ == "__main__":
    folder_path = 'ffurusato_choice'  # フォルダのパスに置き換える
    output_csv = 'gcf_data2.csv'  # 出力するCSVファイル名
